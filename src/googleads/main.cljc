(ns googleads.main
  "Kotodama WASM entrypoint for the Googleads clean-room actor (L4) — Clojure port.
  ...
  facts (`googleads.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "googleads")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"googleads.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "googleads-compat" :tier tier :entities entities} 200])
...
(defn start! [] :googleads-compat/ready)
